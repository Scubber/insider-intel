...
never fail the lane; only systemic listing/parquet failures do). Scanned
PDFs OCR through the command hook; the default backend is the in-image
Tesseract helper (`apps/aggregator/ocr_pdf.py`, binaries only in the
Docker `spark` stage — keyword-grade text is enough for the lexicon scan;
a VLM OCR stays the benched upgrade path). Judgments are English (Art.
348; measured 0/62 Devanagari across 14 benches 2026-08-23) but every
stored row stamps `legal_metadata.language` from its own text and the
cycle log counts `non_english` — the measured trigger for ever building a
Hindi lexicon.
...
- **Corpus lives in the bucket, never in images.** The Dockerfile's final
  ...
  produces it; the deploy workflow and legacy script rely on that). The
  `spark` stage (runtime + tesseract/poppler for the IndiaCourts OCR path,
  targeted by docker-compose.spark.yml) sits before a final `FROM runtime`
  alias stage that preserves that invariant — never let `spark` become the
  default build product.
