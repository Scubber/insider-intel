...
| **Hunt synthesis** | NEW 2026-08-08: refresh job distills each observed technique's case material into generalized hunt patterns (`data/state/technique_hunts.json`, signature-cached, `HUNT_SYNTH_MAX_PER_RUN=10`, chain = summarizer chain/Haiku). Dossier leads with patterns; entity terms (names/companies/domains) are filtered from all hunt surfaces. Initial sweep fills over ~4 days of refreshes. |
